...
void backward(NeuralNetwork* net, double* input, double* hidden, double* output, double* target, double* debugTimes, int imageIndex, int numImages, int epoch, int batchSize) {
    ...
    // Only transferd weights back to host at the end of each epoch or at specified intervals 
    // it is the optimization i  used to reduce weight transfer  overhead because that was the bottleneck
    ...
    if (shouldSyncWeights) {
        ...
        // Don't free the flat arrays since we reuse them
